...
library(readr)
library(tidyverse)
OA_data <- read_csv("./OA_activitydat_20190302.csv")
...
# Using a 'wider' format to assist with further analysis (such as plotting with ggplot)
```
After binding the two datasets together, it was then transformed into a wider format. This was done because the focus was on the changes of fish behaviour depending on the treatment. Thus, for each treatment, we looked at their respective mean, standard deviation and sample size.
