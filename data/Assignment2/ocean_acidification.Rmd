...
Similarly, observations 6, 16, 17, 255, 497, 562 and 564 have "ROM_V" values less than e-04. While assessing the relevant columns, it was found that the standard deviations may be improperly labelled. Both "ctrl.sd" and "oa.sd" values ranges from 0.0001 to over 30,000. This is not characteristic to standard deviation because a distance of range 0.0001 to 30,000 from the mean is highly unlikely. Thus, the standard deviation column is likely to be mislabeled, and should be represented as standard error instead. 
...
fitted to the data that controls for the sampling variance of lnRR. The model should include a random effect of study and observation. Use metafor’s rma.mv() function.
```{r}
